...
olcRootDN: cn=ldap,dc=alv,dc=pub
add one line:
...
al,cn=auth" read by dn.base="cn=ldap,dc=alv,dc=pub" read by * none
 ```
...
```bash
 ...
 ```bash
 ldapadd -x -W -D "cn=ldap,dc=alv,dc=pub" -f /root/base.ldif
  ```
...
```bash
ldapsearch -x -b "dc=alv,dc=pub" -H ldap://ldap.alv.pub
```
###  3. 客户端安装配置调试
```
...
authconfig --enableldap  --enableldapauth --ldapserver=ldap://ldap.alv.pub --disableldaptls  --enablemkhomedir --ldapbasedn="dc=alv,dc=pub" --update
```
然后就可以了。
```bash
